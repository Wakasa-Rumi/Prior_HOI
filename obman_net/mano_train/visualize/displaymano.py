...
def visualize_batch(
    ...
    for row_idx in range(batch_nb):
        ...
        if prior_verts is not None:
            prior_verts3d = get_row(prior_batchverts3d, row_idx)
            gt_prior_verts3d = get_row(gt_prior_batchverts3d, row_idx)
        ...
        if args["model_name"] == "PoseHOI" or args["model_name"] == "Obman":
            add_hand_obj_meshes(
                ax,
                verts3d=verts3d,
                faces_right=hand_faces,
        ...
        if args['model_name'] == 'Obman' or args["model_name"] == "TemplateHOI":
            ...
            add_joints_proj(ax, gt_joints3d, pred_joints3d, proj="z")
            add_scatter_proj(ax, gt_objpoints3d, rot_obj_coarse_pc, proj="z")
            # ax.invert_yaxis()
        else:
            add_hand_obj_meshes(
                ax,
                pred_objpoints3d=rot_obj_coarse_pc,
            )
        ...
        if args['model_name'] == 'PoseHOI' or args['model_name'] == 'Obman':
            center_rot_coarse = rot_obj_coarse_pc - rot_obj_coarse_pc.mean(0)
            center_gt = gt_objpoints3d - gt_objpoints3d.mean(0)
            add_hand_obj_meshes(
                ax,
                pred_objpoints3d=center_rot_coarse,
                gt_objpoints3d=center_gt
            )
        else:
            add_hand_obj_meshes(
                ax,
        ...
        if args["model_name"] == "PoseHOI":
            # input_img = mask_crop[row_idx].numpy()
            # ax.imshow(input_img)
            add_hand_obj_meshes(
                ax,
                pred_objpoints3d=prior_verts3d,
                verts3d=gt_prior_verts3d,
                pred_objfaces=pred_objfaces,
                faces_right=pred_objfaces
            )
        else:
            add_hand_obj_meshes(
                ax,
...
def save_pck_img(thresholds, pck_values, auc_all, save_pck_file, overlay=None):
    ...
    plt.tight_layout()
    plt.savefig(save_pck_file, format="png")
